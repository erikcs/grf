...
CausalSurvivalSplittingRule::~CausalSurvivalSplittingRule() {
  if (counter != nullptr) {
    delete[] counter;
  }
...
bool CausalSurvivalSplittingRule::find_best_split(const Data& data,
                                                  size_t node,
...
void CausalSurvivalSplittingRule::find_best_split_value(const Data& data,
  ...
  std::fill(failure_count, failure_count + num_splits, 0);
  size_t n_missing = 0;
  ...
  for (bool send_left : {true, false}) {
    ...
    for (size_t i = 0; i < num_splits; ++i) {
      ...
      // Skip this split if the left child does not contain enough failures.
      if (num_failures_left < min_child_size_survival) {
        continue;
      }

      // Stop if the right child does not contain enough failures.
