...
rdd_forest <- function(X, Y, Z,
  ...
  args <- list(num.trees = num.trees,
               ...
               stabilize.splits = FALSE, #TODO
               seed = seed)

  forest <- do.call.rcpp(multi_causal_train, c(data, args))
  class(forest) <- c("rdd_forest", "grf")
  forest[["X.orig"]] <- X
  forest[["Y.orig"]] <- Y
  forest[["Z.orig"]] <- Z
...
#' }
#'
#' @method predict rdd_forest
#' @export
predict.rdd_forest <- function(object,
                               newdata = NULL,
                               num.threads = NULL,
                               estimate.variance = FALSE, ...) {
  # If possible, use pre-computed predictions.
  if (is.null(newdata) && !estimate.variance && !is.null(object$predictions)) {
    return(list(predictions = object$predictions[, 1]))
  }
  ...
  list(predictions = ret$predictions[, 1],
